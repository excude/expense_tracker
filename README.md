# v0.09 월 목표금액
- 관리자가 카드별 월 목표금액을 저장하면 뷰어에게도 적용됩니다. 목표는 매월 동일하며 빈칸/0은 미설정입니다.
- 카드 선택창에 월 순사용액 대비 목표 진행바 표시: 100% 미만 녹색, 100% 이상 120% 미만 노란색, 120% 이상 빨간색.
- 전체 카드 목표는 제외하지 않은 카드들의 목표 합계입니다. 해당 월 미사용 카드의 목표도 포함합니다. 전체 순사용액에는 목표 미설정 카드 소비도 포함합니다.
- 막대 길이는 100%에서 멈추지만 실제 달성률은 숫자로 표시합니다. 취소로 음수면 막대는 0으로 표시합니다.

# v0.08 매일 드라이브 자동 동기화
Apps Script setup을 본인 계정으로 한 번 실행해야 활성화됩니다. GitHub Pages 배포만으로 실행되지는 않습니다.
- PROJECT_ID 기본값: fluent-sprite-509500-c0
- BACKUP_FOLDER_ID 미설정 시 `체리피커백업폴더(삭제시 백업/복구 불능)` 이름으로 폴더를 찾아 저장합니다. 같은 이름이 여러 개거나 찾지 못하면 폴더 ID를 직접 입력하세요.
- 최초 즉시 동기화 후 매일 한국시간 오전 4시 15분 전후에 확인합니다. 시간 트리거는 정확한 시각을 보장하지 않습니다.
- 폴더 바로 아래 .db 파일 중 수정 시간이 가장 최근인 파일을 선택합니다. 파일 ID·수정 시각·크기가 같으면 업로드하지 않습니다.
- 드라이브 원본은 읽기 전용입니다. 백업을 옮기거나 지우지 않습니다. 하위 폴더/숨김 앱 데이터/압축 백업은 지원하지 않습니다.
- 기존 syncDrive 트리거는 setup 실행 시 이 스크립트 안에서 교체합니다. 새 프로젝트를 따로 만들었다면 이전 프로젝트의 트리거는 직접 중지하세요.

# v0.07 변경사항
- 백업 올리기 옆 월별 통계 버튼: 제외 카드 설정을 반영한 전체 카드 월별 순사용액 막대그래프.
- 최신 월부터 표시하고 거래가 없는 중간 월은 0원으로 표시합니다. 취소로 음수인 월은 0선 왼쪽 파란 막대로 표시합니다.
- 조회자도 통계를 볼 수 있으며 현재 화면의 월/카드/검색 필터에는 영향을 받지 않습니다.

# v0.06 변경사항
- 월/카드 선택에 모바일 기본 select 창 대신 전용 선택창을 사용합니다. 항목을 누르면 즉시 적용하고 닫습니다. 완료 버튼은 없습니다.

# v0.05 변경사항
- 카드 선택 시 동일한 옵션 DOM을 다시 만들지 않도록 수정해 모바일 선택창 닫힘을 방해하지 않습니다.

# v0.04 변경사항
- 전체 기간 옵션 제거. 기본은 백업의 최신 거래 월이며 월 변경 시 선택을 유지합니다.
- 카드 목록에 선택 월 순사용액(승인+취소)을 표시하고 높은 순으로 정렬합니다. 검색어나 선택 카드에 관계없이 해당 월 전체 거래 기준입니다.
- 관리 화면도 같은 월의 순사용액 순서이며 제외 카드 및 해당 월 미사용 카드(0원)도 관리할 수 있습니다.
- 관리자 닉네임은 Firestore 공통 설정으로 저장하고 뷰어의 카드 목록/거래/검색에도 반영합니다. 비우면 원래 이름으로 돌아갑니다.

# v0.03 변경사항
- 같은 브라우저에서 로그인 상태를 유지합니다. 직접 로그아웃, 브라우저 사이트 데이터 삭제, 시크릿 모드 종료, 계정 세션 취소 등에는 다시 로그인해야 합니다.
- 카드사별 카테고리 기능을 제거했습니다.
- 백업의 card_name과 card_no 문자열 전체가 동일할 때만 카드 항목을 합칩니다. 빈 번호는 합치지 않습니다. 백업이 일부 번호만 저장한 경우 저장된 값까지만 비교할 수 있습니다.
- 거래를 삭제하거나 중복 제거하지 않고, 같은 카드로 모아 합계를 계산합니다.
- 합쳐진 카드 중 하나라도 기존 제외 설정이 있으면 제외를 유지합니다. 관리자에서 다시 체크하면 해당 카드 묶음 전체가 포함됩니다.

# v0.02 변경사항
- editor의 카드 관리 화면에서 제외 카드와 카테고리를 저장합니다.
- 설정은 users/shared/meta/cardSettings에 저장되며 viewer에도 실시간 반영됩니다.
- 제외 카드는 합계, 거래 목록, 카드 선택 목록에서 빠집니다. 원본 백업은 삭제하지 않습니다.
- 카드 선택과 관리 화면은 같은 카테고리별로 묶습니다. 기본값은 백업의 카드사명이며 관리자가 변경할 수 있습니다.
- 기존 Firestore 규칙의 editor 쓰기 / viewer 읽기 권한을 그대로 사용합니다.
- 이 설정은 표시/집계 설정입니다. 원본 백업에 대한 접근 권한을 카드별로 제한하지 않습니다.
- 버전은 v0.02이며 이후 릴리스는 0.01씩 증가시킵니다.
...
5. 최초 동기화 성공 후 매일 한국시간 오전 4시 15분 전후 syncDrive 트리거가 생성됩니다. 실행 내역에서 오류를 확인하세요.
