...
(async()=>{try{
 ...
 function clearView(){settingsStop?.();settingsStop=null;state.settingsReady=false;state.excluded=[];state.nicknames={};state.role=null;$('admin-button').hidden=true;$('card-admin').close();stop?.();stop=null;++generation;state.initialized=false;state.data={records:[],cards:[]};render();$('dashboard').hidden=true;$('login').hidden=false;}
