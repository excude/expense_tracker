...
const state={data:{records:[],cards:[]},page:0,busy:false,initialized:false,settingsReady:false,excluded:[],categories:{},role:null};
...
function cardCategory(card){return state.categories[String(card.id)]||card.category||'미분류';}
function includedRecords(){const excluded=new Set(state.excluded.map(String));return state.settingsReady?state.data.records.filter(r=>!excluded.has(String(r.cardId))):[];}
function updateCardOptions(){const selected=$('card').value,month=$('month').value;const used=new Set(includedRecords().filter(r=>!month||r.date.startsWith(month)).map(r=>String(r.cardId)));const groups=new Map();for(const card of state.data.cards){if(!used.has(String(card.id)))continue;const group=cardCategory(card);if(!groups.has(group))groups.set(group,[]);groups.get(group).push(card);}$('card').replaceChildren(new Option('전체 카드',''));for(const [name,cards] of [...groups].sort((a,b)=>a[0].localeCompare(b[0],'ko'))){const group=document.createElement('optgroup');group.label=name;for(const c of cards)group.append(new Option(c.name,String(c.id)));$('card').append(group);}$('card').value=used.has(selected)?selected:'';}
function drawAdmin(){const root=$('admin-cards');root.replaceChildren();const groups=new Map();for(const c of state.data.cards){const group=cardCategory(c);if(!groups.has(group))groups.set(group,[]);groups.get(group).push(c);}for(const [category,cards] of [...groups].sort((a,b)=>a[0].localeCompare(b[0],'ko'))){root.append(text('h3',category));for(const c of cards){const row=text('div','','admin-card');row.dataset.cardId=String(c.id);const label=text('label','');const check=document.createElement('input');check.type='checkbox';check.checked=!state.excluded.includes(String(c.id));label.append(check,document.createTextNode(c.name));const input=document.createElement('input');input.type='text';input.value=cardCategory(c);input.maxLength=60;input.setAttribute('aria-label',c.name+' 카테고리');row.append(label,input);root.append(row);}}}
...
(async()=>{try{
 ...
 function clearView(){settingsStop?.();settingsStop=null;state.settingsReady=false;state.excluded=[];state.categories={};state.role=null;$('admin-button').hidden=true;$('card-admin').close();stop?.();stop=null;++generation;state.initialized=false;state.data={records:[],cards:[]};render();$('dashboard').hidden=true;$('login').hidden=false;}
 auth.onAuthStateChanged(user=>{
  ...
  accessStop=db.doc('access/'+user.email.toLowerCase()).onSnapshot(member=>{
   ...
   settingsStop=db.doc('users/'+cfg.ledgerId+'/meta/cardSettings').onSnapshot(snapshot=>{if(auth.currentUser?.uid!==identity)return;const settings=snapshot.data()||{};state.excluded=Array.isArray(settings.excludedCardIds)?settings.excludedCardIds.map(String):[];state.categories=settings.categories||{};state.settingsReady=true;state.page=0;$('admin-button').hidden=state.role!=='editor';render();},()=>{state.settingsReady=false;$('admin-button').hidden=true;$('card-admin').close();render();notify('공통 카드 설정을 불러오지 못했습니다. 새로고침해 주세요.',true);});
