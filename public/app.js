...
function parse(bytes){return new Promise((resolve,reject)=>{const worker=new Worker('./parse-worker.js?v=0.03');const timer=setTimeout(()=>{worker.terminate();reject(Error('백업 분석 시간이 초과되었습니다.'));},60000);worker.onmessage=({data})=>{clearTimeout(timer);worker.terminate();data.error?reject(Error(data.error)):resolve(data);};worker.onerror=()=>{clearTimeout(timer);worker.terminate();reject(Error('백업 분석에 실패했습니다.'));};worker.postMessage(bytes,[bytes]);});}
...
function updateCardOptions(){const month=$('month').value;const used=new Set(includedRecords().filter(r=>!month||r.date.startsWith(month)).map(r=>String(r.cardId)));options($('card'),state.data.cards.filter(c=>used.has(String(c.id))).map(c=>({value:String(c.id),label:c.name})),'전체 카드');}
function drawAdmin(){const root=$('admin-cards');root.replaceChildren();for(const c of state.data.cards){const row=text('div','','admin-card');row.dataset.cardId=String(c.id);const label=text('label','');const check=document.createElement('input');check.type='checkbox';check.checked=!cardExcluded(c);label.append(check,document.createTextNode(c.name));row.append(label);root.append(row);}}
...
$('admin-form').onsubmit=e=>{e.preventDefault();run(async()=>{if(state.role!=='editor'||!state.settingsReady)throw Error('관리자 권한이 필요합니다.');const excluded=new Set(state.excluded);for(const row of $('admin-cards').querySelectorAll('.admin-card')){const card=state.data.cards.find(c=>String(c.id)===row.dataset.cardId);for(const id of card.sourceIds||[String(card.id)]){if(row.querySelector('input[type=checkbox]').checked)excluded.delete(String(id));else excluded.add(String(id));}}await db.doc('users/'+cfg.ledgerId+'/meta/cardSettings').set({excludedCardIds:[...excluded],updatedAt:new Date().toISOString()});$('card-admin').close();notify('모든 사용자에게 카드 설정을 저장했습니다.');});};
