...
const state={data:{records:[],cards:[]},page:0,busy:false,initialized:false,settingsReady:false,excluded:[],nicknames:{},targets:{},role:null};
...
function parse(bytes){return new Promise((resolve,reject)=>{const worker=new Worker('./parse-worker.js?v=0.09');const timer=setTimeout(()=>{worker.terminate();reject(Error('백업 분석 시간이 초과되었습니다.'));},60000);worker.onmessage=({data})=>{clearTimeout(timer);worker.terminate();data.error?reject(Error(data.error)):resolve(data);};worker.onerror=()=>{clearTimeout(timer);worker.terminate();reject(Error('백업 분석에 실패했습니다.'));};worker.postMessage(bytes,[bytes]);});}
...
function cardTarget(card){for(const id of [String(card.id),...(card.sourceIds||[])]){const value=Number((state.targets||{})[String(id)]);if(Number.isSafeInteger(value)&&value>0)return value;}return 0;}
function goalProgress(amount,target){if(!(target>0))return null;const ratio=amount/target;return {percent:ratio*100,width:Math.max(0,Math.min(100,ratio*100)),color:amount>=target*1.2?'red':amount>=target?'yellow':'green'};}
function goalElement(amount,target){const box=text('span','','goal-box'),progress=goalProgress(amount,target);if(!progress){box.append(text('span','목표 미설정','goal-caption'));return box;}box.append(text('span','목표 '+won(target)+' · '+progress.percent.toFixed(1)+'%','goal-caption'));const track=text('span','','goal-track');track.setAttribute('role','progressbar');track.setAttribute('aria-valuemin','0');track.setAttribute('aria-valuemax','100');track.setAttribute('aria-valuenow',String(progress.width));track.setAttribute('aria-label','목표 대비 '+progress.percent.toFixed(1)+'%');const fill=text('span','','goal-fill '+progress.color);fill.style.width=progress.width+'%';track.append(fill);box.append(track);return box;}
function appendPickerProgress(button,value){const rows=monthlyCards().filter(x=>!cardExcluded(x.card));if(value===''){button.append(goalElement(rows.reduce((sum,x)=>sum+x.amount,0),rows.reduce((sum,x)=>sum+cardTarget(x.card),0)));}else{const row=rows.find(x=>String(x.card.id)===value);if(row)button.append(goalElement(row.amount,cardTarget(row.card)));}}
function drawAdmin(){const root=$('admin-cards');root.replaceChildren(text('p',($('month').value||'조회 월 없음')+' 순사용액 높은 순 · 제외 카드도 표시'));for(const {card:c,amount} of monthlyCards()){const row=text('div','','admin-card');row.dataset.cardId=String(c.id);const label=text('label','');const check=document.createElement('input');check.type='checkbox';check.checked=!cardExcluded(c);label.append(check,document.createTextNode(c.name+' · '+won(amount)));const input=document.createElement('input');input.type='text';input.maxLength=60;input.value=nickname(c);input.placeholder='닉네임 (비우면 원래 이름)';input.setAttribute('aria-label',c.name+' 닉네임');const targetLabel=text('label','월 목표금액 (원)');const target=document.createElement('input');target.type='number';target.min='0';target.step='1';target.max='1000000000000';target.inputMode='numeric';target.placeholder='미설정';target.value=cardTarget(c)||'';target.setAttribute('aria-label',c.name+' 월 목표금액');targetLabel.append(target);row.append(label,input,targetLabel);root.append(row);}}
...
function openPicker(id){const select=$(id),dialog=$('quick-picker');$('picker-title').textContent=id==='month'?'조회 월 선택':'카드 선택';const list=$('picker-options');list.replaceChildren();for(const option of select.options){const button=text('button',option.textContent,'picker-option');button.type='button';if(id==='card')appendPickerProgress(button,option.value);button.setAttribute('aria-pressed',String(option.value===select.value));button.onclick=()=>{select.value=option.value;dialog.close();select.dispatchEvent(new Event('change',{bubbles:true}));};list.append(button);}dialog.showModal();list.querySelector('[aria-pressed="true"]')?.focus();}
...
(async()=>{try{
 ...
 function clearView(){settingsStop?.();settingsStop=null;state.settingsReady=false;state.excluded=[];state.nicknames={};state.targets={};state.role=null;$('admin-button').hidden=true;$('card-admin').close();$('quick-picker').close();$('monthly-stats').close();stop?.();stop=null;++generation;state.initialized=false;state.data={records:[],cards:[]};render();$('dashboard').hidden=true;$('login').hidden=false;}
